...
page read_page_buf(int table_id, int64_t offset) {
	...
	for (i = 0; i < MAX_FRAME; i++) {
		if (offset == buffer_pool[i]->page_offset && table_id == buffer_pool[i]->table_id) {
			...
			copy_page(&page, &(buffer_pool[i]->page));
			break;
		}
	}
...
int64_t insert_into_leaf_after_splitting(int table_id, int64_t leaf_offset, int64_t key, record *pointer) {
	int64_t new_leaf_offset = make_leaf(table_id);
...
int64_t read_first_leaf_page(int table_id) {
	int64_t offset;
	page table_page = read_page_buf(table_id, 0);
	offset = table_page.root;
	if (offset == 0) {
		return offset;
	}
	
	table_page = read_page_buf(table_id, offset);
	while (!table_page.is_leaf) {
		offset = table_page.offset[0];
		table_page = read_page_buf(table_id, offset);
	}
...
//Implement writing page into txt File
...
int join_table(int table_id_1, int table_id_2, char *pathname) {
	...
	//Set right_page offset variable in leaf_page, File for joining
	int64_t table_id_1_offset = read_first_leaf_page(table_id_1);
	int64_t table_id_2_offset = read_first_leaf_page(table_id_2);
...
int commit_transaction() {
	 ...
	 * IF user get successful return, that means your database can recover committed transaction
	   after system crash.
	 * Return 0 if success, otherwise return non-zero value. */
